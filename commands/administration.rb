...
hear /bot ban (?<lvl>[0-3]) (?<nicks>.+)(?:\: (?<reason>.+))?/i, :dev do
...
hear /kick\s(?<nick>\S+)(?<channel>\s\#\S+)?(?:\s\:\s(?<reason>.+))?/i, :dev do
  send "KICK #{params[:channel]||channel} #{params[:nick]} #{params[:reason]}"
end

hear /kickban\s(\S+)/i, :dev do
  send "KICKBAN #{params[1]}"
end

hear /invite\s(\S+)(?:\s(\S+))?/i, :dev do
